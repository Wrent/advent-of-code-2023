...
fun dayOnePartOne(input: String): Long {
    return input.split("\n").sumOf { getCalibrationValue(it) }
}

private fun getCalibrationValue(input: String): Long {
    val digits = input.split("")
        .mapNotNull { it.toIntOrNull() }
    return (digits.first().toString() + digits.last().toString()).toLong()
}

fun dayOnePartTwo(input: String): Long {
    return input.split("\n").sumOf { getCalibrationValueWithWords(it) }
}
...
internal fun getCalibrationValueWithWords(input: String): Long {
    var str = input
    var first: Int? = null
    while (first == null) {
        val res = extractInt(str, { startsWith(it) }, { first() })
        first = res
        str = str.drop(1)
    }
    str = input
    var last: Int? = null
    while (last == null) {
        val res = extractInt(str, { endsWith(it) }, { last() })
        last = res
        str = str.dropLast(1)
    }
    return (first.toString() + last.toString()).toLong()
}

private fun extractInt(input: String, strFn: String.(String) -> Boolean, charFn: String.() -> Char): Int? {
    return if (input.charFn().isDigit()) {
        input.charFn().digitToInt()
    } else {
        replacements.find { input.strFn(it.first) }?.second
    }
}

private const val input = """dssmtmrkonedbbhdhjbf9hq
