...
fun main() {
    println(dayEightPartOne(input))
    println(dayEightPartTwo(input))
}

fun dayEightPartOne(input: String): Long {
    ...
    while (current != "ZZZ") {
        val instruction = instructions.get(cnt % instructions.size)
        if (instruction == "L") {
...
private fun parse(input: String): Pair<List<String>, Map<String, Node>> {
    val split = input.split("\n\n")

    val instructions = split.get(0).split("")
        .filter { it.isNotBlank() }

    val nodes = split.get(1).split("\n")
        .map { parseNode(it) }
        .map { it.label to it }
        .toMap()
...
private fun parseNode(input: String): Node {
    ...
    val splitDir = split.get(1).drop(1).dropLast(1).split(", ")
    return Node(label, splitDir.get(0), splitDir.get(1))
}
...
fun dayEightPartTwo(input: String): Long {
    val (instructions, nodes) = parse(input)

    val starters = nodes.keys.filter { it.last() == 'A' }
    val nums = starters.map {
        var current = it
        var cnt = 0L

        while (current.last() != 'Z') {
            val instruction = instructions.get(cnt.mod(instructions.size))
            if (instruction == "L") {
                current = nodes[current]!!.left
            } else {
                current = nodes[current]!!.right
