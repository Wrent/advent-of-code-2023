...
fun dayTwoPartOne(input: String, red: Int, green: Int, blue: Int): Int {
    return input.split("\n")
...
fun parseCubeGame(line: String): CubeGame {
    ...
    val takes = cubes.split("; ").map { parseTake(it) }
    return CubeGame(id, takes)
}

fun parseTake(take: String): CubeTake {
    val colors = take.split(", ")
        .associate { it.split(" ").let { it[1] to it[0].toInt() } }
    return CubeTake(
        red = colors["red"] ?: 0,
        blue = colors["blue"] ?: 0,
...
data class CubeGame(
    val id: Int,
    val takes: List<CubeTake>,
) {
    fun green() = takes.maxOf { it.green }
    fun red() = takes.maxOf { it.red }
    fun blue() = takes.maxOf { it.blue }
}
...
fun dayTwoPartTwo(input: String): Long {
    return input.split("\n")
        .map { parseCubeGame(it) }
        .map { it.blue() * it.green() * it.red().toLong() }
        .sum()
}

private const val input = """Game 1: 13 red, 18 green; 5 green, 3 red, 5 blue; 5 green, 9 red, 6 blue; 3 blue, 3 green
