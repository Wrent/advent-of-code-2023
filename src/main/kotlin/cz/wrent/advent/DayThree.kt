...
fun dayThreePartOne(input: String): Int {
    ...
    while (j < chars.size) {
        var i = 0
        while (i < chars[0].size) {
            var current = chars[j][i]
            val digits = mutableListOf<String>()
            var k = 0
            while (current.toIntOrNull() != null) {
                digits.add(current)
                k++
                if (i + k == chars[0].size) {
                    break
            ...
            if (digits.isNotEmpty()) {
                ...
                if (hasSymbolNeighbour(chars, j, i, i + k - 1, number)) {
                    numbers.add(number)
                }
                i += k
            } else {
...
private fun hasSymbolNeighbour(chars: List<List<String>>, row: Int, from: Int, to: Int, number: Int): Boolean {
    var res = false
    for (j in (row - 1).coerceAtLeast(0)..(row + 1).coerceAtMost(chars.size - 1)) {
        for (i in (from - 1).coerceAtLeast(0)..(to + 1).coerceAtMost(chars[0].size - 1)) {
            if (chars[j][i] != "." && chars[j][i].toIntOrNull() == null) {
                res = true
                if (chars[j][i] == "*") {
                    gears.computeIfAbsent(j to i) { mutableListOf() }.add(number)
                }
            }
        }
    }
...
fun dayThreePartTwo(input: String): Long {
    gears.clear()
    dayThreePartOne(input)
    return gears.filter { it.value.size == 2 }.map { it.value[0].toLong() * it.value[1] }.sum()
}
