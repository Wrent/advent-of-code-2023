...
fun main() {
    println(dayTenPartOne(input))
    println(dayTenPartTwo(input))
}

fun dayTenPartOne(input: String): Long {
    val map = getMap(input)
    return getPipe(map).maxOf { it.value }.toLong()
}

fun dayTenPartTwo(input: String): Int {
    val map = getMap(input)
    val pipe = getPipeList(map)

    val area = shoelace(pipe) //https://en.wikipedia.org/wiki/Shoelace_formula
    return area - pipe.size.div(2) + 1 // https://en.wikipedia.org/wiki/Pick%27s_theorem
}

private fun shoelace(list: List<Position>): Int {
    return list.windowed(2, partialWindows = true).sumOf {
        val a = it[0]
        val b = it.getOrNull(1) ?: list[0]

        a.x * b.y - (a.y * b.x)
    }.absoluteValue.div(2)
}
// fun dayTenPartTwo(input: String): Long {
//     val map = getMap(input)
//     val pipe = getPipe(map).clockwise()
//
//     val candidates = map.keys.minus(pipe.keys)
//     val next = mutableSetOf(candidates.first())
...
//         visited.add(current)
//
//         next.addAll(map.dfsNeighbours(current, pipe).minus(visited))
//
//         next.remove(current)
//     }
//
//     return candidates.minus(visited).size.toLong()
// }

private fun getPipe(map: Map<Position, Char>): Map<Position, Int> {
    val start = map.entries.first { it.value == 'S' }.key

    val pipe = mutableMapOf<Position, Int>()
    var dst = 0
    pipe[start] = dst
    var next = map.findNeighbours(start)
    while (next.isNotEmpty()) {
        dst++
        pipe += next.map { it to dst }
        next = next.flatMap { map.neighbours(it) }.toSet().minus(pipe.keys)
    }
    return pipe
}

private fun getPipeList(map: Map<Position, Char>): List<Position> {
    val start = map.entries.first { it.value == 'S' }.key

    val pipe = mutableListOf(start)
    var next = setOf(map.findNeighbours(start).first())
    while (next.isNotEmpty()) {
        pipe += next
        next = next.flatMap { map.neighbours(it) }.toSet().minus(pipe.toSet())
    }
    return pipe
}
...
private fun Map<Position, Char>.findNeighbours(position: Position): Set<Position> {
    return setOf(position.left(), position.right(), position.up(), position.down())
        .filter { this.neighbours(it).contains(position) }
        .toSet()
}

private fun Map<Position, Char>.neighbours(position: Position): Set<Position> {
    val char = this[position] ?: return emptySet()
    return when (char) {
        '.' -> emptySet()
        '-' -> setOf(position.left(), position.right())
        '|' -> setOf(position.up(), position.down())
        'F' -> setOf(position.down(), position.right())
        'L' -> setOf(position.up(), position.right())
        '7' -> setOf(position.down(), position.left())
        'J' -> setOf(position.up(), position.left())
        else -> emptySet()
    }.filter { this.contains(it) }.toSet()
}

// private fun Map<Position, Char>.dfsNeighbours(position: Position, pipe: Map<Position, Int>): Set<Position> {
//     if (!pipe.contains(position)) {
//         return setOf(position.left(), position.right(), position.up(), position.down()).filter { this.contains(it) }.toSet()
//     }
//     val prev = findNeighbours(position).map { it to pipe.getValue(it) }.minBy { it.second }.first
//     val char = this[position]
//     return when (char) {
//         '-' -> setOf(position.left(), position.right())
//         '|' -> setOf(position.up(), position.down())
//         'F' -> if (prev.y > position.y) {
//             setOf(position.down(), position.right(), position.up())
//         } else {
//             setOf(position.down(), position.right(), position.left())
//         }
//         'L' -> if (prev.y < position.y) {
//             setOf(position.up(), position.right(), position.down())
//         } else {
//             setOf(position.up(), position.right(), position.left())
//         }
//         '7' -> if (prev.y > position.y) {
//             setOf(position.down(), position.left(), position.right())
//         } else {
//             setOf(position.down(), position.left(), position.up())
//         }
//         'J' -> if (prev.y < position.y) {
//             setOf(position.up(), position.left(), position.down())
//         } else {
//             setOf(position.up(), position.left(), position.right())
//         }
//         else -> emptySet()
//     }.filter { this.contains(it) }.toSet()
// }

// private fun Map<Position, Char>.clockwise(position: Position, direction: Direction): Pair<Position, Direction> {
//     val char = this[position]
//     return when (char) {
//         '-' -> if (direction == Direction.RIGHT) {
//             position.right() to Direction.RIGHT
//         } else {
//             position.left() to Direction.LEFT
//         }
//         '|' -> if (direction == Direction.DOWN) {
//             position.down() to Direction.DOWN
//         } else {
//             position.up() to Direction.UP
//         }
//         'F' -> if (direction == Direction.UP) {
//             position.right() to Direction.RIGHT
//         } else {
//             position.down() to Direction.DOWN
//         }
//         'L' -> if (direction == Direction.LEFT) {
//             position.up() to Direction.UP
//         } else {
//             position.right() to Direction.RIGHT
//         }
//         '7' -> if (direction == Direction.RIGHT) {
//             position.down() to Direction.DOWN
//         } else {
//             position.left() to Direction.LEFT
//         }
//         'J' -> if (direction == Direction.DOWN) {
//             position.left() to Direction.LEFT
//         } else {
//             position.up() to Direction.UP
//         }
...
// fun dayTenPartTwo(input: String): Long {
//     val map = getMap(input)
//     val pipe = getPipe(map)
//
//     val candidates = (pipe.keys.minBy { it.x }.x..pipe.keys.maxBy { it.x }.x)
//         .flatMap { x ->
//             (pipe.keys.minBy { it.y }.y..pipe.keys.maxBy { it.y }.y)
//                 .map { Position(x, it) }
//         }.toSet()
//         .minus(pipe.keys)
//         .toMutableSet()
//
//     val start = pipe.entries.first { it.value == 0 }.key
//     // val replacedMap = replaceStart(map, start)
//
//     var (current, direction) = findClockwiseNeighbour(start)
//     while (current != start) {
//         removeAllFromCurrent(candidates, current, direction)
//         val next = map.clockwise(current, direction)
//         current = next.first
//         direction = next.second
//     }
//
//     return candidates.size.toLong()
// }
//
// private fun removeAllFromCurrent(candidates: MutableSet<Position>, current: Position, direction: Direction) {
//
// }

// fun replaceStart(map: Map<Position, Char>, start: Position): Map<Position, Char> {
//     val aroundStart = map.findNeighbours(start)
//     val neighbours = aroundStart
//         .map { it to map.neighbours(it) }
//         .filter { it.second.contains(start) }
//         .map { it.first }
//         .map { map.getValue(it) }
//         .toSet()
//
//     val char = when(neighbours) {
//         setOf('-') -> '-'
//         setOf('|') -> '|'
//         setOf('-', -'')
//         else -> error("")
